...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ----------------------------------------------------------------------------
  // 1. DYNAMIC BOOK SPINE RENDERING & RESOLUTION WITH CENTER PLACEMENT
  // ----------------------------------------------------------------------------
  function resolveShelfTrack(genre) {
    if (!genre) return null;
    const directId = genreShelfMap[genre];
    if (directId) {
    ...
    const allShelves = document.querySelectorAll('.shelf-tier, .shelf-cell, .books-container, .shelf-books-track');
    for (const shelf of allShelves) {
      const sGenre = (shelf.getAttribute('data-genre') || shelf.getAttribute('data-shelf-genre') || shelf.getAttribute('data-shelf') || '').toLowerCase();
      if (sGenre && (genreLower.includes(sGenre) || sGenre.includes(genreLower))) {
        return shelf.querySelector('.shelf-books-track, .books-container, .books-row') || shelf;
      }
    }

    return document.getElementById('shelf-scifi');
  }

  function createBookSpineElement(book, isAnimated = false, isFreshArrival = false) {
    ...
    const track = resolveShelfTrack(book.genre) || document.getElementById('shelf-scifi');
    ...
    if (!spine) {
      ...
      // Center placement: insert in the middle of existing spines or before any loader
      const existingSpines = Array.from(track.querySelectorAll('.book-spine'));
      if (existingSpines.length > 0) {
        const middleIndex = Math.floor(existingSpines.length / 2);
        track.insertBefore(spine, existingSpines[middleIndex] || null);
      } else {
    ...
    if (isAnimated || isFreshArrival) {
      spine.classList.add('new-arrival', 'just-shelved');
      setTimeout(() => {
        spine.classList.remove('new-arrival', 'just-shelved');
      }, 7000);
    }
  ...
  function syncShelvesWithCatalog(books) {
    ...
    books.forEach((book, index) => {
      ...
      if (!renderedBookIds.has(bookIdStr)) {
        ...
        // Remove active loader on this book's shelf if present
        const shelfTrack = resolveShelfTrack(book.genre);
        if (shelfTrack) {
          const loader = shelfTrack.querySelector('.shelf-loader');
          if (loader) loader.remove();
        }

        createBookSpineElement(book, isFreshArrival, isFreshArrival);
  ...
  setInterval(pollLibraryCatalog, 3500);
